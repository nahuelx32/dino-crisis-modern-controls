...
- Cámara en tercera persona al hombro (R3 alterna con las cámaras fijas originales):
  ...
  - los cambios de cámara no pausan el juego;
  - fondo negro fuera de la sala;
...
- Correr automático (Cuadrado lo activa o desactiva; R1 camina).
- HUD con arma, cargador/reserva (`VACIO` sin balas) y vitalidad; Triángulo lo oculta.
- Start saltea todos los videos, también los de la intro.
...
- En vistas al hombro muy cargadas puede faltar parte del escenario lejano: es el límite que evita que el juego se cuelgue. Algún marco de puerta puede verse a través de una pared (límite del orden de dibujo del motor).
...
A patch for **Dino Crisis** (PlayStation, Spanish PAL release, **SLES-02211**) that replaces tank controls with modern camera-relative movement and adds an over-the-shoulder third-person camera. It is applied to your own copy of the game; this repository **contains no part of the game**.
...
- Over-the-shoulder camera (R3 toggles with the original fixed cameras):
  - the whole room and the dinosaurs are visible, not only what the fixed camera would show;
  - camera cuts no longer pause the game;
  - black background outside the room;
  - lasers and lights visible from both sides.
...
- Auto-run (Square toggles it; R1 walks).
- HUD with weapon, magazine/reserve (`VACIO` = empty) and health; Triangle hides it.
- Start skips every video, including the intro.
...
The [Redump dump (disc 27940)](http://redump.org/disc/27940/), as `.bin`/`.cue` with its `.sbi`:
...
- **xdelta** (recommended): Delta Patcher or xdelta UI. It refuses to apply to the wrong image.
- **PPF**: PPF-O-Matic. It only checks that it is the same disc, not the exact dump.

Use Track 2, the `.cue` and the `.sbi` unchanged. The patch moves no sectors, so the LibCrypt `.sbi` still works.

**Notes:**

- Tested on PCSX-Redux and DuckStation. Not tested on real hardware: for a console or PS2 (POPStarter), remove LibCrypt with a separate tool before applying the patch.
- Memory card saves work. Savestates from other versions do not.
- In very busy shoulder-camera views some distant scenery may be skipped for that moment: this is the limit that keeps the game from freezing. Some door frames can show through walls (a limit of the engine's draw order).

## Building from source

Linux or WSL with `gcc-mipsel-linux-gnu`, `binutils-mipsel-linux-gnu` and Python 3 (optional: `pip install unicorn` to run the loader as a test). See the Spanish section above for the commands; the build reproduces the release byte for byte. Development notes (`docs/DESARROLLO.md`) are in Spanish.

## Legal

The code in this repository is MIT licensed (see [LICENSE](LICENSE)). *Dino Crisis* is © Capcom. No part of the game is included: no images, executables or disc sectors. You need your own copy.
